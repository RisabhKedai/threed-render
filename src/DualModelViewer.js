...
const DualModelViewerV2 = ({
  roomModelPath, // Path to LivingRoom.ply
  chairModelPath, // Path to PatchWork chair.ply
  roomOrientation = "", // Orientation for the room
  chairOrientation = "", // Orientation for the chair
  ...
  const createMaterial = (
    ...
    const canRenderAsMesh = (hasIndex || hasTriangles) && !forcePoint;
  ...
  // Function to create mesh from geometry - FIXED to use proper rendering logic
  const createMesh = (geometry, material, forcePoint = true) => {
    // Determine if this should be rendered as mesh or points based on geometry structure
    const hasIndex = geometry.index !== null && geometry.index.count > 0;
    const vertexCount = geometry.attributes.position
      ? geometry.attributes.position.count
      : 0;
  ...
  useEffect(() => {
    ...
    plyLoader.load(
      ...
      (geometry) => {
        ...
        const mesh = createMesh(geometry, material); // FIXED: Removed forceMesh parameter
...
export default DualModelViewerV2;
